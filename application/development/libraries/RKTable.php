...
Class RKTable 
{
    ...
    */
    public function generate()
    {
        ...
        {
           $this->heading = array_keys($this->table_data[0]);
        }
        ...
        if(count($this->btn_edit)>0)
        {
            ...
            *   This key should be a field name of the source table, an index of an array, or a unique value on each set of data */
            ...
            /*If the edit button heading is specified */
            if(isset($this->btn_edit['heading']))
            {
                $edit_heading = $this->btn_edit['heading'];
            }
            else
            {
                $edit_heading = "Edit";
            ...
            {
                {
                    array_push($this->heading,$edit_heading); /* add new heading at the beginning */
                    foreach ($this->table_data as $row) /*loop through each rows*/
                    {
                        $key = $row[$edit_key]; /*get the value of specified edit key*/
                        array_push($row, $this->_set_edit_btn($key)); /* add edit button at the beginning of each row */
                        $new_table_data[] = $row;
                    }
                }
            }
            ...
            {
                array_unshift($this->heading, $edit_heading);

                foreach ($this->table_data as $row)
                {
                    $key = $row[$edit_key]; /*get the value of specified edit key*/
                    array_unshift($row, $this->_set_edit_btn($key)); /* add edit button at the end of each row */
                    $new_table_data[] = $row;
                }
            }
        ...
        {
            $new_table_data = $this->table_data;
        }
        ...
        $output .= $this->add_rows($new_table_data);
    ...
    private function _compile_table_data()
    {
        /*return false if table data is not provided or of is empty*/
        if(!isset($this->table_data) OR count($this->table_data)==0){return FALSE;}
        /*set the new container*/
        $new_table_data = array();
        $i=0;
        foreach($this->table_data as $row) /*loop through table data arrays*/
        {
            /*if each row is an object, convert it into an array*/
            $data = is_object($row) ? get_object_vars($row) : $row;
            /*if table array keys is provided and is not empty*/
            if(isset($this->table_array_keys) && count($this->table_array_keys)>0)
            {
                foreach($this->table_array_keys as $key) /*loop through each table array keys*/
                {
                    $new_table_data[$i][$key] = array_key_exists($key, $data) ? $data[$key] : NULL; /*set the value of each key and put into new array*/
                }
            }
            else /*if the table key arrays is not provided put each row into the new array*/
            {
                $new_table_data[$i] = $data;
            }
            $i++;
        }
        $this->table_data = array(); /*empty the global variable table data*/
        $this->table_data = $new_table_data; /*reload the content of new array to the global variable table data*/
        return;
    }
}